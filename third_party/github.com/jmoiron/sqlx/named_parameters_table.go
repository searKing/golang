...
//	// INSERT INTO table (table.foo, table.bar) VALUES (:foo, :bar)
//
...
func NamedTableInsertArgumentsCombined(table string, cols ...string) (arguments string) {
	...
	return fmt.Sprintf(`(%s) VALUES (%s)`, JoinTableColumns(table, cols...), JoinNamedTableValues(cols...))
}
...
func NamedTableInsertArguments(table string, cols ...string) (columns string, arguments string) {
	return JoinTableColumns(table, cols...), JoinNamedTableValues(cols...)
}
...
//	// UPDATE foo SET table.foo=:foo, table.bar=:bar
...
//	// SELECT * FROM table WHERE table.foo=:foo AND table.bar=:bar
//
