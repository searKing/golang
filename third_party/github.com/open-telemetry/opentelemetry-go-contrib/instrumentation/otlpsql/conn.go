...
var (
	...
	_ conn                     = &otlpConn{}
	_ driver.NamedValueChecker = &otlpConn{}
)
...
// otlpConn implements driver.Conn
...
func (c otlpConn) Exec(query string, args []driver.Value) (res driver.Result, err error) {
	...
	if exec, ok := c.parent.(driver.Execer); ok {
		...
		return otlpResult{parent: res, ctx: ctx, options: c.options}, nil
	}
...
func (c *otlpConn) Close() error {
	return c.parent.Close()
}

func (c *otlpConn) Begin() (driver.Tx, error) {
	return c.BeginTx(context.TODO(), driver.TxOptions{})
}

func (c *otlpConn) PrepareContext(ctx context.Context, query string) (stmt driver.Stmt, err error) {
	attrs := append([]attribute.KeyValue(nil), c.options.DefaultAttributes...)
	onDeferWithErr := recordCallStats("go.sql.conn.prepare", c.options.InstanceName)
	defer func() {
		// Invoking this function in a defer so that we can capture
...
func (c *otlpConn) BeginTx(ctx context.Context, opts driver.TxOptions) (tx driver.Tx, err error) {
	attrs := append([]attribute.KeyValue(nil), c.options.DefaultAttributes...)
	...
	if connBeginTx, ok := c.parent.(driver.ConnBeginTx); ok {
		...
		return otlpTx{parent: tx, ctx: ctx, options: c.options}, nil
	}
...
func (c *otlpConn) CheckNamedValue(nv *driver.NamedValue) (err error) {
