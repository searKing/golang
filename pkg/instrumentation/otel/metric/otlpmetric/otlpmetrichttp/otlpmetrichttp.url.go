...
func parseOtlpOpts(q url.Values, opts ...otlpmetrichttp.Option) ([]otlpmetrichttp.Option, error) {
	{
		v := q.Get("compression")
		switch v {
		case "gzip":
...

	{
		...
		switch v {
		case "delta":
			opts = append(opts, otlpmetrichttp.WithTemporalitySelector(func(kind sdkmetric.InstrumentKind) metricdata.Temporality { return metricdata.DeltaTemporality }))
		case "cumulative":
			opts = append(opts, otlpmetrichttp.WithTemporalitySelector(func(kind sdkmetric.InstrumentKind) metricdata.Temporality { return metricdata.CumulativeTemporality }))
		case "default":
