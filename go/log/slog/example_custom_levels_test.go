...
func ExampleHandlerOptions_customLevels() {
	...
	{
		fmt.Printf("----text----\n")
		th := slog.NewTextHandler(os.Stdout, opt)
	...
	{
		fmt.Printf("----glog----\n")
		th := NewGlogHandler(os.Stdout, opt)
	...
	}
	{
		...
		th := NewGlogHumanHandler(os.Stdout, opt)
	...
	// ----glog----
	// EMERGENCY 0] missing pilots
	// ERROR 0] failed to start engines, err=missing fuel
	// WARNING 0] falling back to default value
	// NOTICE 0] all systems are running
	// INFO 0] initiating launch
	// DEBUG 0] starting background job
	// TRACE 0] button clicked
	// ----glog_human----
	// [EMERGENCY] [0] missing pilots
	// [ERROR] [0] failed to start engines, err=missing fuel
	// [WARNING] [0] falling back to default value
