...
func TestScaleRectangleBySize(t *testing.T) {
	tests := []struct {
...
func TestRectangle2f_UnionPoints(t *testing.T) {
	tests := []struct {
		...
		{
			rect:     image_.Rect2f(0, 0, 10, 10),
			pts:      []image_.Point2f{image_.Pt2f(1, 1)},
			wantRect: image_.Rect2f(0, 0, 10, 10),
		},
		{
			rect:     image_.Rect2f(0, 0, 10, 10),
			pts:      []image_.Point2f{image_.Pt2f(-1, -1)},
			wantRect: image_.Rect2f(-1, -1, 10, 10),
		},
		{
			rect:     image_.Rect2f(0, 0, 10, 10),
			pts:      []image_.Point2f{image_.Pt2f(20, 20)},
			wantRect: image_.Rect2f(0, 0, 20, 20),
		},
		{
			rect:     image_.Rect2f(0, 0, 10, 10),
			pts:      []image_.Point2f{image_.Pt2f(-1, 1)},
			wantRect: image_.Rect2f(-1, 0, 10, 10),
		},
		{
			rect:     image_.Rect2f(0, 0, 10, 10),
			pts:      []image_.Point2f{image_.Pt2f(-1, 20)},
			wantRect: image_.Rect2f(-1, 0, 10, 20),
		},
		{
			rect:     image_.Rect2f(0, 0, 10, 10),
	...
	for i, tt := range tests {
		...
		if !tt.wantRect.RoundRectangle().Eq(gotRect.RoundRectangle()) {
			t.Errorf("#%d: %v.UnionPoints(%v) = %v, want %v", i, tt.rect, tt.pts, gotRect, tt.wantRect)
		}
	}
}
