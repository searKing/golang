...
func (c *KetamaNodeLocator) getIterateKeyForNode(node Node, repetition int) string {
	return c.ketamaNodeKeyFormatter.getKeyForNode(node, repetition)
}

func (c *KetamaNodeLocator) getIterateHashKeyForNode(node Node, repetition int) []uint32 {
	return c.hashAlg.Hash(c.getIterateKeyForNode(node, repetition))
}

// 127.0.0.1:11311-0 -> 1122334455
// IterateKey -> IterateHashKey
func (c *KetamaNodeLocator) getHashKey(iterateKey string) uint32 {
