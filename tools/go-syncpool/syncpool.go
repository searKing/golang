...
//
package main // import "github.com/searKing/golang/tools/go-syncpool"
...
var (
	...
	output      = flag.String("output", "", "output file name; default srcdir/<type>_poll.go")
...
func Usage() {
	...
	_, _ = fmt.Fprintf(os.Stderr, "\thttps://godoc.org/github.com/searKing/golang/tools/go-syncpool\n")
	_, _ = fmt.Fprintf(os.Stderr, "Flags:\n")
	flag.PrintDefaults()
}

const (
	goAtomicValueToolName = "go-syncpool"
)

func main() {
	log.SetFlags(0)
	log.SetPrefix("go-syncpool: ")
	flag.Usage = Usage
	flag.Parse()
	if len(*typeInfos) == 0 {
		flag.Usage()
	...
	g.Printf("\n")
	g.Printf(stringImport, "sync") // Used by sync.Pool.
...
func (g *Generator) createValAndNameDecl(val Value) (string, string) {
	...
	nilValName := fmt.Sprintf("_nil_%s_%s_value", val.eleName,
...
func (g *Generator) buildOneRun(value Value) {
	...
	g.Printf("func _() {\n")
	g.Printf("\t// An \"cannot convert %s literal (type %s) to type sync.Pool\" compiler error signifies that the base type have changed.\n", value.eleName, value.eleName)
	g.Printf("\t// Re-run the go-syncpool command to generate them again.\n")
...
const stringOneRun = `func (m *%[1]s) Put(value %[2]s) {
    (*sync.Pool)(m).Put(value)
}

func (m *%[1]s) Get() %[2]s {
    value := (*sync.Pool)(m).Get()
    if value == nil {
    	return %[3]s
    }
    return value.(%[2]s)
